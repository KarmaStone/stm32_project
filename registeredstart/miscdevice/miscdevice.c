...
DEV_HAND c_open( char *name , int32_t flag)
{
	list_iter *iter;
	list_node *node;
	struct miscdevice *misc;
...
int c_read( DEV_HAND fd , uint8_t *data , uint32_t count)
{
...
int c_write( DEV_HAND fd , uint8_t *data , uint32_t count)
{
    DEVICE_OPS *device = (struct miscdevice *)fd;
...
int c_ioctl(FIL_HAND *fd, void *buf, uint32_t count, uint32_t flag)
{
    DEVICE_OPS *device = (struct miscdevice *)fd;
